...
process.argv.forEach((arg) => {
 ...
 - Config file generated by hypestyle-compiler.
 - Compiled on: ${new Date().toLocaleString()}.
 - Compile id: ${id.generate({ length: 8 })}
*/`
        ...
        fetch('https://registry.npmjs.org/hypestyle-compiler/latest').then(
            (res) => {
                res.json().then((data) => {
                    console.log(
                        chalk.cyanBright('\n📦 | Version: ') +
                            chalk.blackBright(data.version)
                    )
                })
            }
        )
